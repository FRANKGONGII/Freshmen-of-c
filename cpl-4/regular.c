...
int Check(char* str1,char* str2,int i,int j,int len1, int len2){
    if(i<len1-1&&j<len2-1){// 还没到最后一位（最多倒数第二位）
        if(str1[i]==str2[j]&&str2[j+1]!='*'&&str1[i]!='\\'){// 直接相等而且后一位不是*的情况
            return Check(str1,str2,++i,++j,len1,len2);
        }
        else if(str1[i]==str2[j]&&str2[j+1]=='*'&&str1[i]!='\\'){// 相等但是可以忽略
            int s = len1-1;
            for(s = len1-1;s>=i;s--){
                if(j+2<=len2-1){
                    if(str1[s]==str2[j+2]){
            ...
            +Check(str1,str2,s,j+2,len1,len2));//贪婪
        }
            ...
            if(str2[j]=='.'&&str2[j+1]!='*'){// 本位是万能字符，利用'.'替代任意字符，而且下一位不是*，不能无限替代
            ...
            else if(str2[j]=='.'&&str2[j+1]=='*'){// 本位是万能字符，而且下一位是*，贪婪模式
                int s = len1-1;
                for(s = len1-1;s>=i;s--){
                    if(j+2<=len2-1){
                        if(str1[s]==str2[j+2]){
                ...
                return Check(str1,str2,s,j+2,len1,len2);
            }
            else if(((str2[j]!='.')&&str2[j]!='\\')&&str2[j+1]=='*'){// 本位不是万能字符，利用*忽略不匹配的字符，
                return Check(str1,str2,i,j+2,len1,len2);
            }
        ...
        for(;j<len2;j+=2){
            if(str2[j]!='\\'&&str2[j+1]=='*'){// 必须要是X*X*X*的形式才可以保证后面的内容不会影响
                continue;
            }
            else{
                return 0;
