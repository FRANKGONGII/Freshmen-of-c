...
void Fail(){
    ...
    Sleep(1000);
    printf("Press r to try again or q to quit the game\n");
    fflush(stdin);
    while(1){
            char cho = 0;
            cho = getch();
            ...
            input = '0';//输入
            now_direct = 'a';//蛇的目前方向
            last_direct = 'a';//蛇上一次的方向，这样设置目的是防止180度运动
            if_food = 0;//场上有没有食物，没有为0，有为1
            score = 0;//得分
            play_time = 0;//时间
            wholeGame();
        }
...
void Food(){
    ...
    int y = 0;
    while(1){ 
        srand((unsigned int)time(NULL)*10000);//一开始设定了时间int为time，找了好久才发现，一定要注意变量名的设置啊。。
        //乘10000是为了增大随机
        x = rand()%98+1;
